...
import (
	...
	"github.com/guowenshuai/ieth/modules/common"
)
...
func InitLogs(config *types.Config) {
	logrus.SetReportCaller(true)

	switch config.BaseConf.LogFormat {
	case LogsText:
	...
	// 输出到日志目录和标准输出
	if config.BaseConf.LogDispatch {
		logrus.SetOutput(ioutil.Discard)
		logrus.AddHook(&WriterHook{
			Writer: MultiWriter(path.Join(config.BaseConf.LogPath, "error")),
		...
		logrus.AddHook(&WriterHook{ // Send info and debug logs to stdout
			Writer: MultiWriter(path.Join(config.BaseConf.LogPath, "info")),
			LogLevels: []logrus.Level{
				logrus.InfoLevel,
				logrus.DebugLevel,
			},
		...
		fileout, err := ConfigLocalFilesystemLogger(path.Join(config.BaseConf.LogPath, "log"))
		if err != nil {
	...
	if config.BaseConf.Debug {
		logrus.SetLevel(logrus.DebugLevel)
		logrus.Debug("Open Debug Level")
	}

	logrus.Debugf("logsFormat is: %+v", config.BaseConf.LogFormat)
}
